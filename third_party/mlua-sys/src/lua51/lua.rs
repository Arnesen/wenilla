...
// benilla: the browser build keeps a 64-bit `lua_Integer` on wasm32 — the vendored Lua defines
// `LUA_INTEGER long long` there (see `third_party/lua-src/lua-5.1.5/luaconf.h`), so the two sides
// agree and `benilla-ui`, written against `Integer = i64`, needs no per-site casts.
