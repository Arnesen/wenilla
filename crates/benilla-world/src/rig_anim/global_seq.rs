...
impl GlobalSeqDrive {
    ...
    /// Pause/resume the joint writes. One caller: the doodad draw gate
    /// (`doodad_anim`'s `set_paused(!drawn)`). While paused the joints hold their last pose;
    /// resume lands on the anchored cursor with nothing to catch up.
