...
impl Plugin for UiPetPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<PetBar>().add_systems(
            Update,
            (
                // The feed rides with the unit feed (before the VM ticks, like the stance bar's);
...
///
/// DISMISS is deliberately not latched: it ends the pet rather than becoming its standing command,
/// and the teardown packet clears the whole bar a moment later. ATTACK additionally raises the
/// attack latch (`0x4bd42e`), which is what lights the button and arms the call-off fork.
fn latch_press(bar: &mut PetBar, entry: PetActionEntry) {
    match entry.kind() {
        benilla_protocol::messages::PET_ACT_COMMAND if entry.action() != PET_COMMAND_DISMISS => {
            bar.spells.state = (bar.spells.state & 0x0800_00FF) | (entry.action() << 8);
            if entry.action() == PET_COMMAND_ATTACK {
                bar.attacking = true;
            }
        }
        benilla_protocol::messages::PET_ACT_REACTION => {
            bar.spells.state = (bar.spells.state & 0xFFFF_FF00) | entry.action();
        }
        _ => {}
    }
}
