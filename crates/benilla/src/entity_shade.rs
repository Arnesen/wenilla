...
/// The shade mix `t` encoding the DAY/NIGHT intensity 1.0 — an indoor entity's target (the
/// reference's interior `[+0xf8] = 1.0`): `intensity = mix(2.5, 0.5, t)` ⇒ `t = 0.75`.
...
pub(crate) struct GroundShade {
    /// Current shade mix (0 = intensity 2.5, 1 = 0.5; [`INDOOR_T`] = the day/night 1.0) — what the
    /// parts' tags show, and what the interior bake fold scales its diffuse word by.
    t: f32,
    /// Where `t` is ramping to (0/1 from the last MCSH sample outdoors; [`INDOOR_T`] indoors).
    target: f32,
    ...
    /// sample is overridden by the day/night target ([`INDOOR_T`]).
...
impl GroundShade {
    ...
    /// day/night 1.0 at [`INDOOR_T`] — the bake fold multiplies its diffuse word by this.
    pub(crate) fn intensity(&self) -> f32 {
        2.5 - 2.0 * self.t
    }

    /// The intensity chase's EFFECTIVE target: indoors the day/night point overrides the MCSH
    /// verdict, and on an outdoor-class WMO surface the LIT point does (`self.target` keeps the
    /// raw sample, so stepping back onto terrain resumes from it).
...
pub(crate) fn update_ground_shade(
    ...
    mut parts: Query<(&mut MeshTag, Option<&InteriorLit>)>,
