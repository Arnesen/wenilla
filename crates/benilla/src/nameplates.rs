...
/// over NPCs"; "we should show our name", 2026-07-12) — the byte-verified binary defaults are
/// both "0"; `UnitNamePlayer` ON is the byte-verified default.
const SHOW_NPC_NAMES: bool = true;
const SHOW_PLAYER_NAMES: bool = true;
const SHOW_OWN_NAME: bool = true;
...
pub(crate) fn drive_nameplates(
    ...
    for (entity, net, guid, tf, store, is_self) in &units {
        ...
        let show = if vplates.0.contains(&entity) || bubbles.0.contains(&entity) {
            ...
            match net.kind {
                EntityKind::Player => SHOW_PLAYER_NAMES,
                EntityKind::Unit => SHOW_NPC_NAMES,
                _ => false,
            }
        };
        if !show {
