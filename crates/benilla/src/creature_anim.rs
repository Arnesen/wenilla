...
/// weapon's Load/Hold clip ([`select::ranged_load_anim`]) — the client's resolver `0x5fd460`,
...
/// It is what sustains the drawn Load→Hold idle between a remote shooter's shots — HoldBow
/// self-loops on `0x200 | 0x400`, HoldRifle/HoldThrown on `0x400` alone (`0x5fc3f0`'s
/// continuation tables; ours folds both bits into one gait gate, the asymmetry unmodeled — a
/// bow's first-GO lands well inside the Load pass either way). Cleared like the client's
/// id-matched `0x60d040` + its unconditional sibling `0x60fc50`: a **non-ranged** visual play by
/// the same caster (the stale-visual cleanup `0x6ec39e`), the local cancel (`0x6ea12b`), and
/// melee attack-start. Deliberately NOT cleared on sheath change or volley end — the client's
/// bit persists latent (a stopped remote shooter keeps holding until its next action; the gait
/// gate needs `CUR == 2` anyway).
#[derive(Component)]
...
pub(crate) struct AnimDriver {
    ...
    deferred: Option<u16>,
    /// The ranged standing idle has completed one pass of its **Load** clip — the idle now
    /// plays the drawn HOLD twin ([`select::ranged_hold_anim`]) until a one-shot (the next
    /// shot's release) resets the nock. INTERIM (decision 0400 §2's dispatch, Q4 — the
    /// `[+0xd58] |= 0x400` hold layer's stand-in).
    ranged_held: bool,
