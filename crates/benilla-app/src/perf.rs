...
impl Plugin for PerfPlugin {
    fn build(&self, app: &mut App) {
        ...
        if std::env::var_os("WOW_MESH_EVENTS").is_some() {
            app.add_systems(Update, count_mesh_events);
        }
        ...
        if std::env::var_os("WOW_CAM_CHANGED").is_some() {
            app.add_systems(bevy::app::PostUpdate, count_camera_changes);
        }
        ...
        {
            app.insert_resource(ArchCensusAt(at));
            app.add_systems(Last, arch_census);
        }
        #[cfg(target_os = "macos")]
...
fn arch_census(world: &mut World) {
