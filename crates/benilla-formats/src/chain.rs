...
//!
//! Later archives override earlier ones for files sharing an internal path (so `patch-2.MPQ` wins); a
//! read resolves a name to the highest-priority archive that holds it. Base content archives carry no
//! `(listfile)`, so resolution is by name **hash**, which works without one.
...
impl Chain {
    /// Open a chain from a vanilla `Data` directory (every archive in [`VANILLA_LOAD_ORDER`] that
    /// exists, lowest priority first) or a single `.MPQ` file (just that archive).
    pub fn open(path: &Path) -> Result<Self> {
        let mut archives = Vec::new();
        if path.is_dir() {
            for name in VANILLA_LOAD_ORDER {
                let mpq = path.join(name);
                if mpq.is_file() {
                    archives.push(
                        Archive::open(&mpq)
                            .with_context(|| format!("opening {}", mpq.display()))?,
                    );
                }
            }
